...
public class PlayerCubes : MonoBehaviour
{
    ...
    void Reset()
    {
        ...
        levelProgressCounter = LevelProgress;
        ...
        CancelInvoke("StartLevelProgress");
        firstTimeOMC = false;
    ...
    void onGameStart()
    {
        ...
        InvokeRepeating(nameof(ReduceBonusCollected), 0, m_ReduceTime);
        InvokeRepeating(nameof(ScoreCounter), 0, 1f);
        InvokeRepeating(nameof(StartLevelProgress), 0, 1f);
        //StartCoroutine(StartLevelProgress());
    }
    ...
    {
        get => PlayerPrefs.GetInt("GamesSinceBestScore", 0);
...
//		}
    ...
    {
        if (m_RigidBody.transform.position.y < 0 /*|| m_RigidBody.transform.position.y > 3*/ &&
            GameManager.Instance.m_GameState == "run")
        {
    ...
    void StopPlayer()
    {
        ...
        ParticleSystem s =
            Instantiate(m_ExplosionParticles, m_RigidBody.transform.position, Quaternion.identity) as ParticleSystem;
    ...
    public void SaveData()
    {
        ...
        LevelProgress = levelProgressCounter;
    ...
    {
        return Mathf.Max(0, Mathf.CeilToInt(Mathf.Log(levelProgressCounter / 100.0f, 2)));
    }

    void CalculateLevelData()
    {
        LevelBar.LevelBarValues data = new LevelBar.LevelBarValues();
        data.level = CalculateCurrentLevel();
        ...
        if (currentLevel != data.level)
        {
            ...
            ParticleSystem s =
                Instantiate(m_LevelExplosion, m_RigidBody.transform.position, Quaternion.identity) as ParticleSystem;
            s.transform.parent = transform;
        }
